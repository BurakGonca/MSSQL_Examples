...
CREATE FUNCTION fncKDVHESAPLA2(@FIYAT DECIMAL,@KATEGORIID INT )
...
SELECT P.ProductName,C.CategoryID,C.CategoryName,p.UnitPrice,(dbo.FNCKDVHESAPLA2(p.UnitPrice,C.CategoryID))
AS [%18 KDVLÝ EKLENMÝÞ HALÝ] FROM Products AS P INNER JOIN Categories AS C ON P.CategoryID=C.CategoryID
GROUP BY P.ProductName,C.CategoryName,p.UnitPrice,C.CategoryID
